...
import { PhotoPlaceholder, T, TagPill, ZelligeMark } from "@/components/ui";
import { useI18n } from "@/lib/i18n";
import { colors, rowDir, shadowDark } from "@/lib/theme";
import { useVenue, useVenueState } from "@/lib/venue";
import { ItemSheet } from "@/components/venue/item-sheet";
...
function MenuInner() {
  ...
  const { restaurant, table, categories, items, groupsByItem, cart } = venue;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.cream, paddingTop: insets.top }}>
      ...
      {count > 0 && (
        <Pressable
          onPress={() => router.push(`/r/${slug}/t/${token}/cart`)}
          style={[
            rowDir(lang),
            shadowDark,
            {

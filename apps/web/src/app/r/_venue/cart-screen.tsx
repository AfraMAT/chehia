...
export function CartScreen() {
    ...
    return (
      <div className="flex flex-col min-h-dvh">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-2 px-8 text-center">
          <span className="font-display font-extrabold text-xl text-ink">{t.cart.empty}</span>
  ...
  return (
    <div className="flex flex-col min-h-dvh">
      ...
      {/* Table context — chosen (scanned or picked) or a prompt to choose (browse) */}
      {table ? (
        <div className="mx-5 mt-3.5 bg-teal-tint rounded-lg px-4 py-3 flex items-center gap-2.5">
          ...
          {Boolean(!table.qr_token) && (
            <button
              type="button"
              onClick={() => setPickerOpen(true)}
              className="text-[12px] font-bold text-teal-pressed underline shrink-0 cursor-pointer"
            >
        ...
        <button
          type="button"
          onClick={() => setPickerOpen(true)}
          className="mx-5 mt-3.5 bg-card border-[1.5px] border-dashed border-line-dashed rounded-lg px-4 py-3 flex items-center gap-2.5 text-start cursor-pointer hover:border-harissa transition-colors"
        >
          <span className="w-6 h-6 rounded-md bg-teal-tint text-teal-pressed flex items-center justify-center text-sm font-extrabold shrink-0">
            ⌖
          </span>
          <span className="flex-1 text-[13px] font-bold text-ink">{t.landing.chooseTable}</span>

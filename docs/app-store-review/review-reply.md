...
_Final for build 1.0 (9). **3 850 characters** — App Store Connect caps the reply box at
4 000, so there is ~150 characters of headroom; if you edit it, re-count. Plain hyphens and
no accented characters on purpose: the ASC message box mangles them._

_Every claim is verifiable in the shipped binary — the permission strings and privacy
manifest were read out of the signed IPA, and app.json has not changed between builds 7, 8
and 9 (only the build number)._

```
Hello, and thank you for the detailed feedback. We have addressed every point.

GUIDELINE 4 (DESIGN) - PERMISSION LANGUAGE
Fixed in build 9. The app requests exactly two permissions: camera (to scan the
QR code printed on the table) and when-in-use location (to list nearby cafes and
confirm the customer is on-site when a venue requires it). Both usage strings are
written in the app's primary localization, French, and are additionally localized
for every supported localization (French, Arabic, English) via InfoPlist.strings,
so the system prompt always matches the app's interface language.

We removed the usage strings for capabilities the app does not use (microphone,
always-on/background location). Those keys are no longer present in the binary.

One string remains that the app never triggers: NSMotionUsageDescription
(localized fr/ar/en). It is declared only because a system framework referenced
by our location library requires it to pass App Store validation (ITMS-90683).
The app never requests motion permission and never reads motion data, so no
motion prompt is ever shown.

Build 9 also fixes Arabic text rendering, restores Reduce Motion support, raises
every control to the 44pt minimum target, and corrects VoiceOver labels.
...
No login is required. The customer app is fully anonymous: no account, no
sign-in. A demo QR code and step-by-step instructions are attached in App Review
Information > Notes.

Two ways in, neither needing a physical QR code:
1. Scan the attached image (demo-qr-cafe-el-marsa-table-12.png) shown on another
   screen. It opens the demo venue "Cafe El Marsa - Table 12".
2. No scanning: tap "Find a place" ("Trouver un etablissement" on a French
   device), select "Cafe El Marsa", pick any table, browse the menu and order.

The demo venue accepts orders at any hour from any location, and location
permission is not required for either path, so neither the time of day nor your
location can block the flow. After sending, a live screen tracks the order
(Received, Preparing, Ready, Served).

GUIDELINE 2.1 - BUSINESS MODEL
1) Does your app access any paid content or services?
   No. No in-app purchases and no paid digital content.
2) What are the paid content or services, and what are the costs?
   None inside the app. Customers pay only for physical food and drink, prepared
   and consumed in person at the cafe. Physical goods, not digital content.
3) Do individual customers pay for the content or services?
   They pay the cafe directly, in person at the counter. No payment happens
   inside the app; it does not process, collect or transmit payment information.
4) If no, does a company or organization pay?
   The cafes are our B2B clients and subscribe to Chehia. That subscription is
   handled entirely outside this iOS app, on our separate web portal
   (business.chehia.app), billed by us directly. It is not accessible or
   purchasable from the app under review.
5) Where do they pay, and what is the payment method?
   Customers: at the venue counter, in person, cash or card - outside the app.
   Businesses: on our web portal, billed directly by us - outside the app.
6) If users create an account, are there fees?
   Customers do not create an account. The app is anonymous and free.
7) Steps for locating in-app purchases:
   There are none. No purchase, subscription or paywall screen exists, and none
   are configured in App Store Connect. The flow is: open the app, scan the table
   QR (or pick a venue and table), view the menu, add items, send the order, pay

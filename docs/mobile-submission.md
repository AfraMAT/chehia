...
> ⚠️ Values below are verified against `apps/mobile/app.json` and `eas.json` as of 2026-07-27.
> This section was previously wrong on `buildNumber`, the associated domain, the motion string
> and the reviewer button label — check the files, not this list, before quoting it to Apple.
...
- **app.json** — `ios.buildNumber "6"` (the next production build auto-increments to 7), `android.versionCode 1`, `ITSAppUsesNonExemptEncryption:false`, `supportsTablet:false` (iPhone app; runs on iPad in compatibility mode), iOS **privacy manifest** — required-reason APIs for AsyncStorage/file-timestamp/boot-time/disk **plus the four collected data types** that match the ASC App Privacy label (Name, Other User Content, User ID, Purchase History; all *not* linked, no tracking). Usage strings: camera, when-in-use location **and motion** — all three trilingual via `expo.locales`. Universal links are `applinks:app.chehia.app` (**not** `chehia.app`) plus the Android `autoVerify` intent filter on `app.chehia.app/r`.
- **The motion string is deliberate.** `NSMotionUsageDescription` exists only because Apple's delivery scan (ITMS-90683) requires it — a framework linked by expo-location references the motion API. The app never requests motion permission and never reads motion data. **Never remove it, and never tell Apple it was removed.**
- **eas.json** — `development`/`preview` (dev Supabase) + `production` (prod Supabase) build profiles, `autoIncrement`, and a `submit.production` block that is **fully filled in** (`appleId`, `ascAppId` `6787508673`, `appleTeamId` `9KSK39WBM6`).
...
- **Reviewer path** — the home screen's **"Find a place"** (fr: "Trouver un établissement", ar: "اعثر على مكان") opens Discovery, so a reviewer can browse a venue, pick a table, and order **without a physical QR**. The string is `home.findRestaurant` — the *key* still says "restaurant" but the *label* does not. Quote the label, never the key.

## Prerequisites (your accounts — one-time)
1. **Apple Developer Program** — ✅ active (Team ID `9KSK39WBM6`, `abbesmoez22@gmail.com`). App ID `tn.chehia.app` and the App Store Connect record both **exist** (`ascAppId` `6787508673`).
...
3. **EAS / Expo account** — free; `npm i -g eas-cli && eas login`. Currently signed in as `aframat`.

## Deep-link verification (already configured — verify, don't re-fill)
`eas.json` → `submit.production.ios` needs nothing more; all three fields are set.

The well-known files are served by the **web** app and must resolve on the domain the app
actually associates, which is `app.chehia.app` — **not** the apex `chehia.app`:
  - `APPLE_TEAM_ID` (Vercel env) → `https://app.chehia.app/.well-known/apple-app-site-association`
  - `ANDROID_CERT_SHA256` = EAS upload-key SHA-256 **and** Play app-signing SHA-256 (comma-separated)
    → `https://app.chehia.app/.well-known/assetlinks.json`

Checking the apex instead of `app.` is the mistake this doc used to encode: the apex serves the
same route, so it returns 200 and looks fine while proving nothing about the associated domain.
...
**The authoritative, paste-ready reviewer notes live in
[`docs/app-store-review/review-reply.md`](app-store-review/review-reply.md) §A** — they are kept
verbatim in sync with the shipping build's real button labels. Do not paste from here; the short
version below is orientation only.

> Chehia is a QR table-ordering app. A reviewer without a physical QR code opens the app, taps
> **"Find a place"** ("Trouver un établissement" on a French device), chooses the demo venue
> **Café El Marsa**, picks a table, adds items and places an order — this exercises the full
> flow. Payment happens in person at the counter; there is no in-app payment and no account.

⚠️ Any label quoted to Apple must be copied from `packages/shared/src/i18n/{fr,ar,en}.ts` at the
commit the build was cut from. This doc previously said "Find a restaurant", which no button has
said for several builds — a reviewer following it lands on a screen with no such control, which is
exactly how a Guideline 2.1 "we could not locate the feature" rejection happens.

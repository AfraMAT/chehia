...
The customer app (`apps/mobile`, Expo SDK 57) is an **anonymous, order-only, pay-at-counter** app: no accounts, no in-app payment, one permission (camera, for QR scanning). That keeps review simple — it sidesteps Sign in with Apple (no third-party login), IAP (physical goods/services), and most privacy obligations.
...
1. **Apple Developer Program** — $99/yr (`developer.apple.com`). Create the App ID `tn.chehia.app` and an app record in App Store Connect.
...
- `eas.json` → `submit.production.ios`: `appleId`, `ascAppId` (App Store Connect app ID), `appleTeamId`.
...
- **Data collected:** order contents + table (linked to an anonymous session id, not to a person). No name, email, phone, or precise location is required. Camera is used **on-device** to read the QR — no images are stored or transmitted.
